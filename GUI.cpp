...
std::vector<ALLEGRO_BITMAP*> IMGCache;
std::vector<ALLEGRO_BITMAP*> IMGFilelist;
std::vector<std::unique_ptr<std::string>> IMGFilenames;
GLhandleARB tinter;
...
{
    Core::printerr("%d\n", ssConfig.maxAnnouncementsShown);;
    for (int i = announcements.size() - 1; i >= (announcements.size() - ssConfig.maxAnnouncementsShown) && announcements[i]->duration > 0; i--)
    {
        int offset = ((announcements.size() - 1) - i) * al_get_font_line_height(font);
        //Core::printerr("%d\n", i);;
        //Core::printerr("%s\n", DF2CONSOLE(announcements[i]->text).c_str());
...
int loadImgFile(const char* filename)
{
    if (ssConfig.cache_images) {
        ...
        */
        ...
        IMGFilenames.push_back(std::make_unique<std::string>(filename));
        al_set_separate_blender(op, src, dst, alpha_op, alpha_src, alpha_dst);
        if (ssConfig.saveImageCache) {
